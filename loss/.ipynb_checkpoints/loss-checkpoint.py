...
from loss.Gradient import gradient 
...
class GradientLoss(nn.Module):
    ...
    def forward(self, sr, hr):
        sr_g = self.gradient((sr+1) * 127.5)
        hr_g = self.gradient((hr+1) * 127.5)
...
class AdversarialLoss(nn.Module):
    def __init__(self, logger, use_cpu = False, gan_k = 1, lr_dis = 1e-4, img_size = 160):
    ...
    def forward(self, fake, real):
        ...
        # Training Discriminator k times
        for _ in range(self.gan_k):
            ...
            outputs          = d_hat.sum(), inputs = hat,retain_graph = True, 
...
def get_loss_dict(args, logger):
    ...
    loss['adv_loss'] = AdversarialLoss(logger = logger, use_cpu = args.cpu, gan_k = args.GAN_k, 
                                       lr_dis = args.lr_rate_dis, img_size = args.img_training_size)
    return loss