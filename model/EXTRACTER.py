...
class Extracter(nn.Module):
    ...
    def SearchBlocks(self, lr_lv3, refsr_lv3, ref_lv1, ref_lv2, ref_lv3): 
        batch, c,  h, w  = lr_lv3.size()
        Q = F.pad(lr_lv3, (1, 1, 1, 1), mode='replicate')       
        ## ---- Non-overlaping blocks ----
        Q  = F.unfold(lr_lv3,   kernel_size = self.unfold_kernel_size_lv3, 
                                    padding = 0, 
                                     stride = self.unfold_kernel_size_lv3)        
        K = F.unfold(refsr_lv3, kernel_size = self.unfold_kernel_size_lv3, 
        ...
        S, H = torch.max(R, dim = 1)                  # [N, H*W]  

        # Unfold reference features
        ...
        # select most relevant testures
        K_lv3_unfold = self.bis(K_, 2, H)
        T_lv3_unfold = self.bis(V_lv3_unfold, 2, H)
        T_lv2_unfold = self.bis(V_lv2_unfold, 2, H)
        T_lv1_unfold = self.bis(V_lv1_unfold, 2, H)

        Q_lv3 = F.fold(Q_,                output_size = (h, w),                      
                                         kernel_size = self.unfold_kernel_size_lv3, 
                                             padding = 0, 
                                              stride = self.unfold_kernel_size_lv3)
            
        K_lv3 = F.fold(K_lv3_unfold, output_size = (h, w),                      
                                          kernel_size = self.unfold_kernel_size_lv3, 
                                             padding = 0, 
                                              stride = self.unfold_kernel_size_lv3)

        T_lv3 = F.fold(T_lv3_unfold, output_size = (h, w),                      
                                          kernel_size = self.unfold_kernel_size_lv3, 
                                             padding = 0, 
                                              stride = self.unfold_kernel_size_lv3)
        T_lv2 = F.fold(T_lv2_unfold, output_size = (2 *h, 2 * w), 
                                          kernel_size = self.unfold_kernel_size_lv2, 
                                             padding = 0, 
                                              stride = self.unfold_kernel_size_lv2)
        T_lv1 = F.fold(T_lv1_unfold, output_size = (4 * h, 4 * w), 
                                          kernel_size = self.unfold_kernel_size_lv1, 
                                             padding = 0, 
                                              stride = self.unfold_kernel_size_lv1) 
                
        return K_lv3, Q_lv3, T_lv3, T_lv2, T_lv1
    ...
    def Transfer(self, h, w, soft, hard, V_lv3, V_lv2, V_lv1) :
        ...
        for i in range(hard.shape[1]): 
            ...
            S_     = soft[:, i, :].view(soft[:, i, :].size(0), 1, h // self.stride_lv3 - 2 , w // self.stride_lv3 - 2)
    ...
    def SearchTransfer(self, lr_lv3, refsr_lv3, ref_lv1, ref_lv2, ref_lv3):
        ...
        K, Q, V3, V2, V1 = self.SearchBlocks(lr_lv3, refsr_lv3, ref_lv1, ref_lv2, ref_lv3)
        S_, H_        = self.SearchPatches(Q, K)
        S, T3, T2, T1 = self.Transfer(h, w, S_, H_, V3, V2, V1)
