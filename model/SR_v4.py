class SR(nn.Module):
    ...
    def forward(self, x,  S = None, T_lv3 = None, T_lv2 = None, T_lv1 = None):
        ...
        f_lv1 = 0
        for i in range(len(T_lv3)) :
            S1           = F.interpolate(S[i], size = (x11.shape[2], x11.shape[3]), mode='bicubic')
            x11_res      = torch.cat((x11, T_lv3[i] * S1), dim = 1)
            x11_res      = self.conv11_head[i](x11_res)
            f_lv1        = f_lv1 + x11_res
        ...
        f_lv2 = 0
        for i in range(len(T_lv3)) :
            S2           = F.interpolate(S[i], size= (x22.shape[2], x22.shape[3]), mode='bicubic')
            x22_res      = torch.cat((x22, T_lv2[i] * S2), dim = 1)
            x22_res      = self.conv22_head[i](x22_res)
            f_lv2        = f_lv2 + x22_res
        ...
        f_lv3 = 0
        for i in range(len(T_lv3)) :
            S3           = F.interpolate(S[i], size= (x33.shape[2], x33.shape[3]), mode='bicubic')
            x33_res      = torch.cat((x33, T_lv1[i] * S3), dim = 1)
            x33_res      = self.conv33_head[i](x33_res) 
            f_lv3        = f_lv3 + x33_res
